...
class Log_Ratio_HC():
    ...
    def train_ratio(self, data, target_policy):
            ...
            constraint  = -(neg_KL + self.kl_max_lim)

            #main function
            loss = neg_KL + self.Lamda*(constraint)
            ...
            #optimizing(maximizing) for the lamda
            lamda_loss  = -self.Lamda*(constraint).detach()
            #lamda_loss = -loss.detach()
            ...
            self.current_KL = (neg_KL_unweigheted.item() , neg_KL.item(), self.Lamda)
