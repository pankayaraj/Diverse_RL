...
class Log_Ratio_LE():
    ...
    def __init__(self, nu_param, algo_param, deterministic_env=True, averege_next_nu = True,
                 ...
                 kl_max_lim = 20, decay_rate = 2, lim_frac = 0.9):
        ...
        self.nu_network_limit = Nu_NN(nu_param, save_path=save_path, load_path=load_path)
        self.nu_optimizer = torch.optim.Adam(self.nu_network_limit.parameters(), lr=self.nu_param.l_r)

        self.hard_update(source=self.nu_network_limit, target=self.nu_network)
    ...
    def check_for_limit(self, episode_batch, target_policy):

        #for episode_batch in episode_batchs:
        log_ratio = self.get_log_state_action_density_ratio(episode_batch, target_policy, limiter_network=True)
        #log_ratio2 = self.get_log_state_action_density_ratio(episode_batch, target_policy, limiter_network=False)

        KL = log_ratio.sum().item()
        #KL2 = log_ratio2.sum().item()


        if np.linalg.norm(KL) < self.kl_max_lim:
            self.hard_update(source=self.nu_network_limit, target=self.nu_network)
        else:
            self.hard_update(source=self.nu_network, target=self.nu_network_limit)
    ...
    def train_ratio(self, data, target_policy):
            ...
            #loss = torch.nn.functional.mse_loss(torch.Tensor([13]).to(self.nu_param.device), neg_KL)
            loss = (13+neg_KL)**2
            ...
            self.current_KL = (neg_KL_unweigheted.item(), neg_KL.item())
    ...
    def compute(self, state, action, next_state, next_action, initial_state, initial_action, target_policy):
        ...
        nu = self.nu_network_limit(state, action)
        initial_nu = self.nu_network_limit(initial_state, initial_action)
        ...
        if self.average_next_nu and self.discrete_poliy:
            ...
            all_next_nu = [self.nu_network_limit(next_state, one_hot_next_action[action_i])
                           for action_i in range(self.nu_param.action_dim)]
            ...
            next_nu = self.nu_network_limit(next_state, next_action)
    ...
    def compute_for_eval(self, state, action, next_state, target_policy, limiter_network=True):
        ...
        if limiter_network:
            nu_network = self.nu_network_limit
        else:
            nu_network = self.nu_network
